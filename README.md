...
Debt-to-Income (DTI) Ratio: $$\text{DTI} = \frac{\text{Total Monthly Debt}}{\frac{\text{Annual Income}}{12}}$$

Loan-to-Income (LTI) Ratio: $$\text{LTI} = \frac{\text{Loan Amount Requested}}{\text{Annual Income}}$$

Gini Coefficient: $$\text{Gini} = 2 \times \text{ROC-AUC} - 1$$

Probability of Default (PD): $$P(Y = 1 | X) = \sigma(W^T X + b) \quad \text{or} \quad \text{XGBoost Tree Ensemble}$$

Credit Score Mapping: $$\text{Score} = \text{Base Score} - (\text{PD} \times \text{Scaling Factor})$$

How to Run in Google Colab

Open the interactive notebook: https://colab.research.google.com/drive/1pIXLxjN1z2OcGk76hUccPJ1Qa2QNW1aW?usp=sharing
Configure borrower profile inputs (Income, Loan Amount, Monthly Debt, Delinquencies) in the interactive HUD.
Run all cells sequentially to train the models, generate performance charts, and view the automated underwriting decision.
